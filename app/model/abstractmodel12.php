...
use Root\database\DatabaseHandler;

class AbstractModel12
{
    ...
    // TODO:: Check the valid dates in MYSQL to created a proper pattern
    ...
    private  function created(array $pattern = null)
    {
        $sql = 'INSERT INTO ' . static::$tableName . ' SET ' . $this->buildNameParametersSQL($pattern);
    ...
    private  function update(array $pattern = null)
    {
        $sql = 'UPDATE ' . static::$tableName . ' SET ' . $this->buildNameParametersSQL($pattern) . ' WHERE ' . static::$primaryKey . ' = ' . $this->{static::$primaryKey};
        $stmt = DatabaseHandler::factory()->prepare($sql);
        $this->prepareValues($stmt);
        return $stmt->execute();
    }
    ...
    public function save($primaryKeyCheck = true)
    {
        ...
        return $this->{static::$primaryKey} === null ? $this->created() : $this->update();
    }
